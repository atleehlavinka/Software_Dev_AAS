...
//            if($email == false) {
//                $field->setMessage('Invalid email address.');
//            }
        ...
        public function birthDate($name, $value, $required = true) {
            ...
            $datePattern = '/^(0?[1-9]|1[012])\/'
                    . '(0?[1-9]|[12][[:digit:]]|3[01])\/'
                    . '[[:digit:]]{4}?$/';
            $message = 'Invalid date format345.';
            $this->pattern($name, $value, $datePattern, $message);
            
            if  ($field->hasError()) {
                return;
            }
            
            $dob_split = explode('/', $value);
            $month = $dob_split[0];
            $day = $dob_split[1];
            $year = $dob_split[2];
            if (strlen($month) == 1) {
            ...
            $now = new \DateTime();
            if ($birthDate >= $now) {
                $field->setErrorMessage('Date must be less than today');
                return;
            }
